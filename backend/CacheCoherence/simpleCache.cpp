  ...
  switch (cacheline.state){
    case MODIFIED:
      for (int i = 0; i < NUM_PROCESSORS; i++){
        if (i != core && write) cacheline.state[core] = INVALID;
      }
      break;
    case EXCLUSIVE:
      if (write) cacheline.state[core] = MODIFIED;
      for (int i = 0; i < NUM_PROCESSORS; i++){
        if (i != core && write) cacheline.state[core] = INVALID;
        else if (i != core && !write) cacheline.state[core] = SHARED;
      }
      break;
    case SHARED:
      if (write){
        cacheline.state[core] = MODIFIED;
        for (int i = 0; i < NUM_PROCESSORS; i++){
          if (i != core && write) cacheline.state[core] = INVALID;
        }
      }
    ...
    case INVALID:
      ...
      cacheline.state[core] = EXCLUSIVE;
      for (int i = 0; i < NUM_PROCESSORS; i++){
        if (cacheline.state[i] == MODIFIED || cacheline.state[i] == EXCLUSIVE){
          cacheline.state[i] = SHARED;
          cacheline.state[core] = SHARED;
          break;
        }
        if (i != core && cacheline.state[i] == SHARED){
          cacheline.state[core] = SHARED;
          break;
        }
      }
